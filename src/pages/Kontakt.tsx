...
const Kontakt = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    toast({ title: t.kontakt.form.successTitle, description: t.kontakt.form.successMessage });
  ...
  useEffect(() => {
    ...
    if (subject) { setFormData(prev => ({ ...prev, message: `Ich interessiere mich für: ${subject}\n\n` })); }
  }, [location]);
                  ...
                  <motion.div variants={cardItem}>
                    <Card className="p-8 shadow-soft">
                      ...
                      <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <Button type="submit" variant="gold" size="lg" className="w-full font-semibold">{t.kontakt.form.submit}</Button>
                      </form>
                    </Card>
                  </motion.div>
                  ...
                  <motion.div variants={cardItem}>
                    <Card className="p-8 bg-gradient-teal text-white">
                      ...
                      <div className="space-y-6">
                        <div className="flex items-start gap-4"><MapPin className="w-6 h-6 flex-shrink-0 mt-1" /><div><p className="font-semibold mb-1">{t.kontakt.info.standort}</p><p>Toulouse, Frankreich</p></div></div>
                        <div className="flex items-start gap-4"><Phone className="w-6 h-6 flex-shrink-0 mt-1" /><div><p className="font-semibold mb-1">{t.kontakt.info.telefon}</p><p>+49 162 170 9979</p></div></div>
                        <div className="flex items-start gap-4"><Mail className="w-6 h-6 flex-shrink-0 mt-1" /><div><p className="font-semibold mb-1">E-Mail</p><p className="break-all">contact@johanneschrist.com</p></div></div>
                  ...
                  <motion.div variants={cardItem}>
                    <Card className="bg-white rounded-2xl p-4 sm:p-6 shadow-xl border border-accent/10">
                      <div className="text-center mb-4">
                        <h4 className="font-heading text-xl text-primary mb-1.5 flex items-center justify-center gap-2"><Brain className="w-5 h-5 text-accent" />{t.angebot?.formate?.map?.therapie?.title || "Gestalttherapie"}</h4>
                        <div className="inline-block px-3 py-1.5 bg-accent/10 text-accent font-medium text-xs rounded-lg mb-4">IZICure, 47 Allées Jean Jaurès, 31000 Toulouse</div>
                      </div>
                      <div className="w-full h-[200px] rounded-xl overflow-hidden shadow-inner border border-accent/20">
                        <iframe src="https://maps.google.com/maps?q=IZICure,+47+All.+Jean+Jaur%C3%A8s,+31000+Toulouse&t=m&z=17&output=embed&iwloc=Near" width="100%" height="100%" style={{ border: 0 }} allowFullScreen loading="lazy" referrerPolicy="no-referrer-when-downgrade" title="IZICure Location" />
                      </div>
                    </Card>
                  </motion.div>
                  <motion.div variants={cardItem}>
                    <Card className="bg-white rounded-2xl p-4 sm:p-6 shadow-xl border border-accent/10">
                      <div className="text-center mb-4">
                        <h4 className="font-heading text-xl text-primary mb-1.5 flex items-center justify-center gap-2"><Dumbbell className="w-5 h-5 text-accent" />{t.angebot?.formate?.map?.training?.title || "Personal Training"}</h4>
                        <div className="inline-block px-3 py-1.5 bg-accent/10 text-accent font-medium text-xs rounded-lg mb-4">12 Rue Jean-Palaprat, 31000 Toulouse</div>
                      </div>
                      <div className="w-full h-[200px] rounded-xl overflow-hidden shadow-inner border border-accent/20">
                        <iframe src="https://maps.google.com/maps?q=12+Rue+Jean-Palaprat,+31000+Toulouse&t=m&z=17&output=embed&iwloc=Near" width="100%" height="100%" style={{ border: 0 }} allowFullScreen loading="lazy" referrerPolicy="no-referrer-when-downgrade" title="Personal Training Location" />
                      </div>
                    </Card>
                  </motion.div>
                </motion.div>
