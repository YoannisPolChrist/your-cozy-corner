...
import { Hand, Brain, Palette, ArrowRight, Star, Dumbbell, MapPin, CheckCircle, RefreshCw, UserMinus, Compass, Award } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main>
          ...
          <div className="container mx-auto px-4">
            <div className="text-center mb-16">
              <span className="text-accent text-sm uppercase tracking-[0.2em] font-medium mb-4 block">
                {t.shared?.gestaltPainLabel ?? 'Die Ausgangslage'}
              </span>
              <h2 className="typ-h2 text-primary mb-4">
                {t.gestalttherapie.painPoints.title}
              </h2>
            </div>
                ...
                <AnimatedItem key={idx}>
                  <Card className="p-8 bg-off-white border-none shadow-sm hover:shadow-md transition-all h-full relative group">
                    ...
                    <div className="relative z-10">
                      ...
                      <p className="typ-body text-muted-foreground italic">
                        {item.description}
                      </p>
                    </div>
                  </Card>
        ...
        {/* Approach & Qualifications Section */}
          ...
          <div className="container mx-auto px-4">
            <div className="max-w-6xl mx-auto">
              <StaggerContainer className="grid lg:grid-cols-2 gap-12 items-center">
                {/* Left Side: Text & Qualifications */}
                <AnimatedItem className="space-y-8">
                  <div>
                    <span className="text-accent text-sm uppercase tracking-[0.2em] font-medium mb-4 block">
                      {t.gestalttherapie.approach?.label}
                    </span>
                    <h2 className="typ-h2 text-primary mb-6">
                      {t.gestalttherapie.approach?.title}
                    </h2>
                    <p className="typ-body text-muted-foreground whitespace-pre-line mb-8">
                      {t.gestalttherapie.approach?.description}
                    </p>
                  </div>
                ...
                <AnimatedItem className="relative w-full">
                  <div className="grid grid-cols-2 gap-4 md:gap-6">
                    {/* Column 1 - shifted down slightly for masonry effect */}
                    <div className="space-y-4 md:space-y-6 pt-8 md:pt-12">
                      <motion.img
                        whileHover={{ scale: 1.02 }}
                        transition={{ duration: 0.4 }}
                        src={johannesCoachingTalk}
                        alt="Gestalttherapie Coaching"
                    ...
                    <div className="space-y-4 md:space-y-6">
                      <motion.img
                        ...
                        alt="Gestalttherapeut Johannes Christ"
                        className="w-full rounded-2xl shadow-lg object-cover"
                        loading="lazy"
                      />
                      <motion.img
                        whileHover={{ scale: 1.02 }}
                        transition={{ duration: 0.4 }}
                        src={johannesCoachingNew}
                        alt="Gestalttherapie Sitzung"
                        className="w-full rounded-2xl shadow-lg object-cover"
                        loading="lazy"
                      />
                    </div>
        ...
        {/* Neues Philosophie-Statement: Worum es wirklich geht */}
          ...
          <div className="container mx-auto px-4 relative z-10">
            <AnimatedSection className="max-w-4xl mx-auto space-y-12">
              <div className="text-center">
                <span className="text-accent text-sm uppercase tracking-[0.2em] font-medium mb-4 block">
                  {t.gestalttherapie.philosophy.label}
                </span>
                <div className="flex flex-wrap justify-center gap-3 mb-6">
                  {t.gestalttherapie.philosophy.badges.map((badge, idx) => (
                    <span key={idx} className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-accent/30 bg-accent/10 text-accent text-xs font-semibold uppercase tracking-wider">
                      {badge}
                    </span>
                  ))}
                </div>
                <h2 className="typ-h2 mb-10">
                  {t.gestalttherapie.philosophy.title}
                </h2>
                ...
                <div className="mt-16 md:mt-24 p-8 md:p-12 border border-accent/20 rounded-2xl bg-white/5 relative">
                  ...
                  <p className="typ-h4 text-accent font-light leading-relaxed">
                    {t.gestalttherapie.philosophy.quote}
                  </p>
                </div>
              </div>
            </AnimatedSection>
        ...
        {/* Testimonials Sektion (Rezensionen) */}
        <AnimatedSection className="py-24 md:py-32 bg-off-white relative overflow-hidden">
          <div className="absolute inset-0 bg-primary/[0.02]" />
          <div className="container mx-auto px-4 relative z-10">
            <div className="text-center mb-16">
              <span className="text-accent text-sm uppercase tracking-[0.2em] font-medium mb-4 block">
                {t.shared?.testimonialsLabel ?? 'Erfahrungen'}
              </span>
              <h2 className="typ-h2 text-primary mb-4">
                {t.shared?.testimonialsTitle ?? 'Stimmen meiner Klienten'}
              </h2>
              <p className="typ-body text-muted-foreground max-w-2xl mx-auto">
                {t.shared?.gestaltTestimonialsSubtitle ?? 'Integrität und Vertraulichkeit haben höchste Priorität. Hier ein paar anonymisierte Einblicke.'}
              </p>
            </div>
              ...
              {testimonials.map((testimonial, index) => (
                <AnimatedItem key={index}>
                  <Card className="p-8 bg-white shadow-soft border-none h-full flex flex-col">
                    ...
                    <p className="text-lg text-primary italic leading-relaxed mb-8 flex-grow">
                      "{testimonial.text}"
                    </p>
        ...
        <AnimatedSection className="py-20 md:py-28 bg-white">
          <div className="container mx-auto px-4">
            <div className="max-w-3xl mx-auto text-center">
              ...
              <h2 className="typ-h2 text-primary mb-4">
                {t.shared?.gestaltLocationTitle ?? 'Praxis für Gestalttherapie'}
              </h2>
              <p className="typ-h3 font-heading text-accent mb-4">
                {t.gestalttherapie.location?.city ?? 'Toulouse Zentrum'}
              </p>
              <p className="typ-body text-muted-foreground">
                {t.gestalttherapie.location?.description ?? 'IZICure, 47 Allées Jean Jaurès, 31000 Toulouse'}
              </p>
              ...
              <motion.div
                ...
                className="mt-8 rounded-2xl overflow-hidden shadow-lg border border-accent/10 h-[350px] w-full"
              >
                <iframe
        ...
        {/* Pricing Section (matching Angebot.tsx's einzelbegleitung/intensiv) */}
        <section className="py-24 md:py-32 bg-secondary/30" id="pricing">
          <div className="container mx-auto px-4">
            <div className="text-center mb-16">
              <span className="text-accent text-sm uppercase tracking-[0.2em] font-medium mb-4 block">
                {t.shared?.pricingLabel ?? 'Transparenz'}
              </span>
              <h2 className="typ-h2 text-primary mb-4">
                {t.shared?.gestaltPricingTitle ?? 'Konditionen & Investition'}
              </h2>
            </div>
            ...
            <StaggerContainer className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
              ...
              <AnimatedItem>
                <Card className="p-8 bg-white shadow-soft border-none h-full relative overflow-hidden flex flex-col">
                  ...
                  <p className="typ-body text-muted-foreground mb-8 text-left whitespace-pre-line flex-grow">
              ...
              <AnimatedItem>
                <Card className="p-8 bg-primary text-white shadow-xl h-full relative overflow-hidden border-0 flex flex-col">
                  ...
                  <p className="typ-small text-white/70 mb-4 line-through">
                    {t.shared?.regularPrice ?? 'Regulär'} {t.angebot.konditionen.einzelbegleitung.price}
                  </p>
                  <p className="typ-body text-white/85 mb-8 flex-grow">
        ...
        {/* Cross-Link to Personal Training - Premium Transition */}
        <AnimatedSection className="py-24 md:py-32 bg-off-white relative overflow-hidden">
          <div className="absolute inset-0 bg-primary/[0.02]" />
          <div className="container mx-auto px-4 relative z-10">
            <StaggerContainer className="max-w-4xl mx-auto">
              ...
              >
                ...
                <div className="absolute inset-0 bg-primary">
                  ...
                  <motion.div
                    className="absolute -top-32 -right-32 w-96 h-96 rounded-full bg-gold-accent/20 blur-[100px]"
                    animate={{ scale: [1, 1.2, 1], opacity: [0.3, 0.6, 0.3] }}
                    transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }}
                  />
                  <motion.div
                    className="absolute -bottom-32 -left-32 w-80 h-80 rounded-full bg-teal-highlight/20 blur-[80px]"
                ...
                <div className="relative z-10 p-10 md:p-16 flex flex-col md:flex-row items-center gap-8 md:gap-12">
                  ...
                  <div className="text-center md:text-left flex-grow">
                    <span className="text-gold-accent text-sm uppercase tracking-[0.2em] font-semibold mb-3 block">
                      {t.shared?.crossLinkLabel ?? 'Die andere Dimension'}
                    </span>
                    <h3 className="typ-h3 text-white mb-4">
                      {t.shared?.crossLinkTrainingTitle ?? 'Eher auf der Suche nach körperlicher Veränderung?'}
                    </h3>
                    <p className="text-white/80 text-lg leading-relaxed max-w-xl">
                      {t.shared?.crossLinkTrainingDescription ?? 'Manchmal beginnt die mentale Transformation am besten über den eigenen physischen Körper. Entdecke mein evidenzbasiertes Personal Training.'}
                    </p>
                  </div>
