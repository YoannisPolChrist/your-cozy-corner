        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="max-w-5xl mx-auto">
          <div className="grid md:grid-cols-3 gap-12 md:gap-8 mb-12">
            ...
            <div className="md:text-right">
              <p className="font-heading text-lg text-primary font-bold mb-6 relative inline-block">{t.footer.rechtliches}<span className="absolute -bottom-2 md:right-0 md:left-auto left-0 w-8 h-0.5 bg-gold-accent rounded-full"></span></p>
              <nav className="space-y-3 text-sm flex flex-col items-start md:items-end">
                <Link to={getLocalizedPath('/datenschutz')} className="text-muted-foreground hover:text-gold-accent transition-colors py-1">{t.footer.datenschutz}</Link>
                <Link to={getLocalizedPath('/impressum')} className="text-muted-foreground hover:text-gold-accent transition-colors py-1">{t.footer.impressum}</Link>
              </nav>
            </div>
          </div>
          <div className="pt-8 border-t border-black/5 text-center flex flex-col md:flex-row justify-between items-center gap-4">
            <p className="text-xs text-muted-foreground/80 font-medium">© {new Date().getFullYear()} {t.footer.copyright}</p>
            <div className="flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-gold-accent opacity-50"></span><span className="w-1.5 h-1.5 rounded-full bg-teal-primary opacity-50"></span></div>
          </div>
        </motion.div>
      </div>
    </footer>
