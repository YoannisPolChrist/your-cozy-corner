document.addEventListener('DOMContentLoaded', () => {
    ...
    document.body.dataset.motion = reducedMotionQuery.matches ? 'reduced' : 'full';
    ...
    function syncCanonicalUrl(replace) {
        const canonical = canonicalSlugs[currentLang];
        if (!canonical) return;
        ...
        window.history[method]({}, '', canonical);
    }

    function pulseContainer(className) {
        if (!appContainer || reducedMotionQuery.matches) return;
        appContainer.classList.remove(className);
        void appContainer.offsetWidth;
        appContainer.classList.add(className);
        window.setTimeout(() => appContainer.classList.remove(className), 520);
    }

    function setSuccessState(visible) {
        if (navActions) navActions.style.display = visible ? 'none' : 'flex';
        if (successActions) {
            successActions.style.display = visible ? 'flex' : 'none';
            successActions.classList.toggle('is-visible', visible);
        }
        if (trustBadge) {
            trustBadge.style.display = visible ? 'flex' : 'none';
            trustBadge.classList.toggle('is-visible', visible);
        }
    }

    function goToStep(targetIndex, options = {}) {
        const direction = options.direction || (targetIndex > currentStepIndex ? 'forward' : 'backward');
        const instant = Boolean(options.instant) || reducedMotionQuery.matches;

        if (targetIndex < 0 || targetIndex >= steps.length) return;
        if (targetIndex === currentStepIndex && !options.force) {
        ...
        const exitClass = direction === 'backward' ? 'step-exit-backward' : 'step-exit-forward';
        const enterClass = direction === 'backward' ? 'step-enter-backward' : 'step-enter-forward';

        setSuccessState(false);

        if (instant) {
            currentStep.classList.remove('active', 'step-exit-forward', 'step-exit-backward', 'step-enter-forward', 'step-enter-backward');
            nextStep.classList.remove('step-exit-forward', 'step-exit-backward', 'step-enter-forward', 'step-enter-backward');
        ...
        currentStep.classList.remove('step-enter-forward', 'step-enter-backward');
        currentStep.classList.add(exitClass);

        window.setTimeout(() => {
            currentStep.classList.remove('active', exitClass);
            nextStep.classList.remove('step-exit-forward', 'step-exit-backward', 'step-enter-forward', 'step-enter-backward');
            currentStepIndex = targetIndex;
            nextStep.classList.add('active', enterClass);
            updateUI();
            window.setTimeout(() => {
    ...
    nextBtn.addEventListener('click', () => {
        if (currentStepIndex < steps.length - 1) {
            goToStep(currentStepIndex + 1, { direction: 'forward' });
        } else {
            setSuccessState(true);
            pulseContainer('success-glow');
        }
    });

    prevBtn.addEventListener('click', () => {
        if (currentStepIndex > 0) {
            goToStep(currentStepIndex - 1, { direction: 'backward' });
        }
    });
    ...
    stepDots.forEach(dot => {
        dot.addEventListener('click', () => {
            ...
            goToStep(target, { direction: target > currentStepIndex ? 'forward' : 'backward' });
        });
    });
    ...
    function updateUI() {
        ...
        document.body.dataset.step = String(currentStepIndex + 1);
        if (appContainer) appContainer.dataset.step = String(currentStepIndex + 1);
        ...
        try {
            ...
            if (dataObj['_currentStepIndex'] !== undefined) {
                ...
                if (!isNaN(stepIdx) && stepIdx >= 0 && stepIdx < steps.length) {
                    goToStep(stepIdx, { instant: true, force: true });
                }
            }
